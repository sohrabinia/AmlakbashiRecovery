...
filterSubject.addListener(updateRegionUI);
...
function updateRegionUI(newData) {
    ...
    $('div#sortItem' + newData.sort).addClass("sort-item-selected");
    if (newData.area != -1) {
        $('#divRegionFilter').css("background-color", "#fdd835");
        $('#divRegionFilter span').text($('.js-area-select option:selected').text());
    }
    else if (newData.city != -1) {
        $('#divRegionFilter').css("background-color", "#fdd835");
        $('#divRegionFilter span').text($('.js-city-select option:selected').text());
    }
    else if (newData.province != -1) {
        $('#divRegionFilter').css("background-color", "#fdd835");
        $('#divRegionFilter span').text($('.js-province-select option:selected').text());
    }
    else if (newData.countryDirection == 1) {
        $('#divRegionFilter').css("background-color", "#fdd835");
        $('#divRegionFilter span').text('شمال');
    }
    else {
        $('#divRegionFilter').css("background-color", "white");
        $('#divRegionFilter span').text('ایران');
    }
}
