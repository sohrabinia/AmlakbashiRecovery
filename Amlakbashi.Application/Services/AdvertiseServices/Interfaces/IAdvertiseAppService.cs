...
namespace Amlakbashi.Application.Services.AdvertiseServices.Interfaces
{
    public interface IAdvertiseAppService
    {
        ...
        IList<Advertise> GetAdvertisesByStatus(Advertise.AdvertiseStatus status, bool haveSlug = false);
        ...
        AdvertiseDirector GetAdvertisePageData(long id, out Dictionary<Advertise.AdvertiseType, IList<AdvertiseDirector>> childrenDirectors);
        ...
        AdvertiseDirector SubmitExtraForm(Advertise data, out Dictionary<string, string> errors, out List<string> groupErrors, out int level, IFormFile uploadedLicenseFile, IList<int> tagsId, bool isEdit = false);
        ...
        AdvertiseDirector GetComplexForm(long id, long parentId, out Advertise.AdvertiseType parentType, out bool isEdit);
        AdvertiseDirector SubmitComplexForm(Advertise data, int userId, out Dictionary<string, string> errors,
            out List<string> groupErrors, bool save, out Advertise.AdvertiseType parentType, string rootPath);
        AdvertiseDirector GetAdminForm(long id, DirectorType type, out Advertise.AdvertiseType parentType, out Advertise.AdvertiseStatus status);
        ...
        AdvertiseDirector SubmitAdminForm(Advertise data, out Dictionary<string, string> errors,
            ...
            out Advertise.AdvertiseType parentType, out Advertise.AdvertiseStatus status,
            IList<int> tagsId = null, IFormFile uploadedLicenseFile = null);
        Dictionary<Advertise.AdvertiseType, Dictionary<long, string>> GetAccChilds(long parentId);
        ...
        void Publish(long id, int doerUserId, ActionLog.ActionSourceEnum actionSource);
        void Suspend(long id);
        Task<ServiceResult<Advertise.AdvertiseStatus>> UpdateActivityAsync(long residenceId);
        ...
        Task<bool> UpdateInstantReserveStatusAsync(long residenceId, Advertise.InstantReserveStatusEnum status);
        ...
        IList<Advertise> GetAccListByIds(IList<long> ids, Advertise.AdvertiseStatus status = Advertise.AdvertiseStatus.Unset);
        ...
        void SetHygieneProtocol(long id, Advertise.HygieneProtocolStatus value);
