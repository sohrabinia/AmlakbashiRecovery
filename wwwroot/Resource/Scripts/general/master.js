...
function ShowMobile(phoneNumber, accId) {
    const formattedInnerText = phoneNumber.replace('+98 ', '0');
    const formattedHref = phoneNumber.replace('+98 ', '+98');
    ...
    $.ajax({
        ...
        success: function (ret) {
            if (ret.status == 1) {
                ...
                console.log('error at addShowMobileCounter')
            }
        }
    });

    if (phoneNumber != "NotAuth") {
        ...
        let content = `
            <div style="text-align:  center;">
                ...
                <a href="tel:${formattedHref}" onclick="copyToClipboard('${formattedHref}')">${formattedInnerText}</a>
            </div>
        `;

        showInfoMessage('شماره تماس میزبان', content);
    } else {
        toggle_login();
    }
}
