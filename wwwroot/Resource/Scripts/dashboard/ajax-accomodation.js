...
function setAccActive(id, active, elem) {
    ...
    if (!btn.is(":checked")) {
        ...
        myajax("accomodation/toggleactive", "id=" + id + "&active=" + active, function (ret) {
            ...
            else if (ret.msg != null && ret.msg != "") {
                showInfoMessage('اخطار', ret.msg);
            }
        });
    }
}
...
function pinAcc(id) {
    showInfoMessage('نردبان کردن آگهی ' + id, '', { contentUrl: '/accomodation/Pin?residenceId=' + id });
}

function LastChanceAcc(id) {
    showInfoMessage('لحظه آخر کردن آگهی ' + id, '', { contentUrl: '/accomodation/LastChance?residenceId=' + id });
}